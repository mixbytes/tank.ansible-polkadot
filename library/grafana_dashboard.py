...
options:
  ...
  commit_message:
    description:
      - Set a commit commit_message for the version history.
    ...
    if dashboard_exists is True:
        if dashboard == payload:
            ...
            if 'commit_message' in data and data['commit_message']:
                payload['commit_message'] = data['commit_message']
            ...
            if info['status'] == 200:
                ...
                raise GrafanaAPIException('Unable to update the dashboard %s : %s' % (uid, body['commit_message']))
    else:
        # create
        r, info = fetch_url(module, '%s/api/dashboards/db' % data['grafana_url'], data=json.dumps(payload), headers=headers, method='POST')
        if info['status'] == 200:
